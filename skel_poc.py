...
import numpy as np
from anatomical_atlas import load_atlas_registered, ensure_bodyparts3d_atlas, bodyparts3d_atlas_status
import pandas as pd
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    result={'version':'110.3.20.15-segment-gentle-anatomical-layers','retargeting_mode':'fullbody_segment_frame_reference_skel_skeleton_layer','scale':float(scale.cpu()),
...
def _skel_volumetric_anatomy_2010(joints, joint_names):
    """V110.3.20.15 · Generates volumetric anatomically-shaped bone and muscle meshes.
...
def _v2013_atlas_for_mesh(mesh_seq):
    """Carga mallas anatómicas reales. V110.3.20.15 prioriza BodyParts3D cacheado en runtime."""
    import os as _os
    ...
    if _status.get("ready"):
        return load_atlas_registered(_status["root"],J,names)
    root=_os.path.join(_os.path.dirname(__file__),"assets","anatomical_atlas")
    return load_atlas_registered(root,J,names)
...
def _v2013_render_video_mp4(mesh_seq, layer="skin", fps=25):
    ...
    if layer in ("bones","muscles","all"):
        atlas=_v2013_atlas_for_mesh(mesh_seq)
    ...
    try:
        for t in range(Vviz.shape[0]):
            ...
            if layer in ("bones","all"):
                for p in bone_parts:
                    VV=np.asarray(p["V"],dtype=np.float32)
                    FF=np.asarray(p["F"],dtype=np.int32)
                    if VV.ndim!=3 or len(FF)==0: continue
...
def _plot_skel_mesh_walker(mesh_seq):
    """V110.3.20.15 · visor de atlas anatómico REAL sobre SKEL. Sin fallback geométrico."""
    try:
        ...
        if not _atlas_status.get("ready"):
            st.warning("V110.3.20.15 · Atlas BodyParts3D real todavía no está instalado en esta instancia.")
            st.caption("Las mallas se descargan a caché temporal del runtime; no se guardan en Supabase y no se añaden al NPZ del paciente.")
            if st.button("🦴💪 Preparar atlas anatómico real BodyParts3D",key="v2015_install_bp3d",use_container_width=True):
                with st.spinner("Descargando y preparando huesos/músculos anatómicos reales…"):
                    try:
                        _r=ensure_bodyparts3d_atlas(force=False)
                        st.session_state["v2015_atlas_install_result"]=_r
                        if _r.get("ok"):
                            st.success(f"Atlas preparado · {_r.get('available',0)} estructuras reales · {_r.get('downloaded',0)} descargadas ahora.")
                            st.rerun()
                        else:
        ...
        _atlas_status=bodyparts3d_atlas_status()
        if _atlas_status.get("ready"):
            st.success(f"✅ Atlas anatómico real activo · {_atlas_status.get('bones',0)} huesos · {_atlas_status.get('muscles',0)} músculos")
            st.caption("Fuente: BodyParts3D / Anatomography · CC BY-SA 2.1 Japan. " +
                       "BodyParts3D, © The Database Center for Life Science.")
            atlas=load_atlas_registered(_atlas_status["root"],Jviz,names)
        else:
            atlas_root=_os.path.join(_os.path.dirname(__file__),"assets","anatomical_atlas")
            atlas=load_atlas_registered(atlas_root,Jviz,names)
        if atlas.get("missing"):
            st.warning("Algunas estructuras anatómicas no están disponibles o no pudieron mapearse a los joints SKEL.")
            st.caption("Pendientes: " + ", ".join(atlas["missing"][:30]) + (" …" if len(atlas["missing"])>30 else ""))
        bone_parts=atlas.get("bones",[]); muscle_parts=atlas.get("muscles",[])
        mins=np.nanmin(Vviz.reshape(-1,3),axis=0); maxs=np.nanmax(Vviz.reshape(-1,3),axis=0); span=np.maximum(maxs-mins,1e-3); pad=np.maximum(.08*span,.03)
        ranges=[[float(mins[d]-pad[d]),float(maxs[d]+pad[d])] for d in range(3)]
        P={"V":np.round(Vviz,5).tolist(),"F":F[:,:3].tolist(),"ids":ids,"ranges":ranges,
           "bones":[{"id":x["id"],"V":np.round(x["V"],5).tolist(),"F":x["F"].tolist()} for x in bone_parts],
           "muscles":[{"id":x["id"],"V":np.round(x["V"],5).tolist(),"F":x["F"].tolist()} for x in muscle_parts]}
        payload=_json.dumps(P,separators=(",",":")); js=get_plotlyjs()
        html=f"""<!doctype html><html><head><meta charset='utf-8'><style>html,body{{margin:0;background:white;font-family:system-ui}}#bar{{display:flex;gap:6px;align-items:center;padding:6px;flex-wrap:wrap}}button{{padding:6px 10px;border:1px solid #cbd5e1;background:white;border-radius:5px}}#slider{{flex:1;min-width:280px}}#plot{{height:710px;width:100%}}</style><script>{js}</script></head><body>
        <div id='bar'><button onclick='layer("skin")'>🧍 Piel</button><button id='bb' onclick='layer("bone")'>🦴 Huesos reales</button><button id='bm' onclick='layer("muscle")'>💪 Músculos reales</button><button id='ba' onclick='layer("all")'>Todas</button><button onclick='play(1)'>▶1×</button><button onclick='pause()'>⏸</button><b id='counter'>Frame 1/{len(ids)}</b><input id='slider' type='range' min='0' max='{len(ids)-1}' value='0'></div><div id='plot'></div>
        <script>
        const P={payload},gd=document.getElementById('plot'),sl=document.getElementById('slider'),co=document.getElementById('counter');let k=0,on=false,last=0,raf=0;
        function xyz(A,n){{let q=A[n];return[q.map(p=>p[0]),q.map(p=>p[1]),q.map(p=>p[2])]}}
        let s=xyz(P.V,0),sf=P.F.map(x=>x[0]),sg=P.F.map(x=>x[1]),sh=P.F.map(x=>x[2]);
        const data=[{{type:'mesh3d',x:s[0],y:s[1],z:s[2],i:sf,j:sg,k:sh,name:'Piel SKEL',opacity:.22,visible:true}}];
        const boneIdx=[],muscleIdx=[];
        for(const part of P.bones){{let q=xyz(part.V,0),f=part.F;boneIdx.push(data.length);data.push({{type:'mesh3d',x:q[0],y:q[1],z:q[2],i:f.map(x=>x[0]),j:f.map(x=>x[1]),k:f.map(x=>x[2]),name:part.id,opacity:1,visible:true}})}}
        for(const part of P.muscles){{let q=xyz(part.V,0),f=part.F;muscleIdx.push(data.length);data.push({{type:'mesh3d',x:q[0],y:q[1],z:q[2],i:f.map(x=>x[0]),j:f.map(x=>x[1]),k:f.map(x=>x[2]),name:part.id,opacity:.72,visible:false}})}}
        Plotly.newPlot(gd,data,{{title:'V110.3.20.15 · Anatomical Atlas Mesh',margin:{{l:0,r:0,t:45,b:0}},scene:{{aspectmode:'data',xaxis:{{range:P.ranges[0]}},yaxis:{{range:P.ranges[1]}},zaxis:{{range:P.ranges[2]}}}}}},{{responsive:true,displaylogo:false}});
        function layer(m){{Plotly.restyle(gd,{{visible:(m==='skin'||m==='all')}},[0]);if(boneIdx.length)Plotly.restyle(gd,{{visible:(m==='bone'||m==='all')}},boneIdx);if(muscleIdx.length)Plotly.restyle(gd,{{visible:(m==='muscle'||m==='all')}},muscleIdx)}}
        function frame(n){{k=(n+P.V.length)%P.V.length;let q=xyz(P.V,k);Plotly.restyle(gd,{{x:[q[0]],y:[q[1]],z:[q[2]]}},[0]);let z=0;for(const p of P.bones){{q=xyz(p.V,k);Plotly.restyle(gd,{{x:[q[0]],y:[q[1]],z:[q[2]]}},[boneIdx[z++]])}}z=0;for(const p of P.muscles){{q=xyz(p.V,k);Plotly.restyle(gd,{{x:[q[0]],y:[q[1]],z:[q[2]]}},[muscleIdx[z++]])}}sl.value=k;co.textContent='Frame '+P.ids[k]+'/'+P.ids[P.ids.length-1]}}
        sl.oninput=()=>{{on=false;frame(+sl.value)}};function loop(t){{if(!on)return;if(!last)last=t;if(t-last>33){{frame(k+1);last=t}}raf=requestAnimationFrame(loop)}}window.play=function(){{on=true;last=0;cancelAnimationFrame(raf);raf=requestAnimationFrame(loop)}};window.pause=function(){{on=false;cancelAnimationFrame(raf)}};window.layer=layer;
        </script></body></html>"""
        ...
        if not bone_parts or not muscle_parts:
            st.info("Los botones de vídeo Huesos/Músculos permanecen desactivados hasta que existan mallas anatómicas reales OBJ/STL en el atlas. La Piel sí puede exportarse ahora mismo.")
    except Exception as exc:
...
def _load_mesh_sequence_npz_bytes_2012(raw):
    """V110.3.20.15 · Reabre un NPZ SKEL ya calculado sin volver a generar los 75 frames."""
    try:
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.caption("V110.3.20.15 · Carga el NPZ descargado de este registro para abrir directamente sus 75 frames SKEL. No repite fitting, retargeting ni generación de los 75 frames.")
